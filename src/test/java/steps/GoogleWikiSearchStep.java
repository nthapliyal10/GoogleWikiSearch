...
public class GoogleWikiSearchStep extends TestBase {
    ...
    public void verify_that_coordinates_information_is_correct() {
        Assert.assertTrue(wikipediaSearchResultPage.getCoordinateValue().contains("52.4"));
        Assert.assertTrue(wikipediaSearchResultPage.getCoordinateValue().contains("13.8"));
    }
    ...
    public void open_new_tab_for_google_maps_and_verify_the_coordinates(String mapsUrl) {
        ...
        for (String win : tabs) {
            if (!mainWindow.equals(win)) {
                ...
                Assert.assertEquals(googleMapsSearchPage.getSearchResultNameText().getText(), prop.getProperty("gmap.location"));
